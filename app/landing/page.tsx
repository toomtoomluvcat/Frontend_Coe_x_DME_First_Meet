    ...
    try {
      ...
      if (isAxiosError(err) && err.response?.status === 404) {
        switch (attemp) {
          ...
          case 4:
            setShowError(
              "ยังไม่หยุดอีกหรอ เดิ๋ยวจะโดนดีนะ ถ้าแน่ก็เข้ามาเลย! 😤"
            );
            break;
          default:
            setSubmit(true);
          ...
          <div className="mb-8  sm:mx-4 mx-0 border border-[#880000] ">
            <div className="py-2 px-4 bg-[#880000] text-white flex justify-between items-center">
              <h3 className=" text-[0.6em] sm:text-[0.8em]">
                งาน Coe x DME First Meet คืออะไร
              </h3>
              <p
                className="text-[0.7em] sm:text-[1em]"
                onClick={() => setShowsuggestiom(false)}
              >
                x
              </p>
            </div>
            <h2 className="bg-[#FFF2EB] px-4 py-4 text-[#880000] sm:text-[0.8em] text-[0.7em]  font-thin indent-[20px] text-justify">
              งาน CoE x DME First Meet เป็นกิจกรรมที่จัดขึ้นเพื่อให้นักศึกษาสาขา
              CoE และ DME ได้ทำความรู้จักกัน พูดคุย แลกเปลี่ยนประสบการณ์
              และฝึกการทำงานเป็นทีม โดยจัดที่อาคารเพียรวิจิตร ชั้น 3 ห้อง
              EN16307
              <br className="" />
              <p>
                {" "}
                แม้กิจกรรมนี้จะมีของที่ระลึกและส่งเสริมความสัมพันธ์ระหว่างเพื่อนและพี่ๆ
                ในสาขา แต่การไม่เข้าร่วมจะ ไม่ส่งผลต่อการเรียน หรือข้อกำหนดใดๆ
                ของคณะและมหาวิทยาลัย เป็นกิจกรรมเสริมที่มาแล้วอบอุ่นแน่นอน!
              </p>
            </h2>
          </div>
        )}
          ...
          {joinTeam && status == "authenticated" ? (
            <div className="flex flex-col items-center gap-y-4">
              <Image
                ...
                alt="happy_party"
              ></Image>
              <h2 className="font-medium text-[1.3em] text-[#880000]">
                กลุ่ม {groupName}
              </h2>
            ...
            <div className="flex flex-col">
              <h2 className="font-normal text-center mb-4 text-[#880000]">
                กรอกรหัสของทีมด้านล่าง
              </h2>
              <input
                placeholder="กรอกรหัสทีมของคุณ"
                value={code}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setCode(e.target.value)
                }
                className="text-[#880000] w-[200px] mx-auto border-2 border-[#880000] focus:outline-none px-2 py-1"
                type="text"
              />
              <button
                type="button"
                onClick={() => joinTeamByCode()}
                className="text-[#FFF2EB] w-fit rounded-[5px] text-[0.8em] mt-4 mx-auto py-2 px-8 bg-[#880000]"
                disabled={submit}
              >
                {submit ? "กำลังส่ง" : "ยืนยัน"}
              </button>
              <p className="text-center text-[0.9em] mt-2  text-[#880000]">
                {showError}
              </p>
            </div>
          )}
