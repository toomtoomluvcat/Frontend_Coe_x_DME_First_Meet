                ...
                <div className="w-full  space-y-4">
                    {teamData.map((item) => (
                        <div key={item.ID} className="text-white border border-[#5599AA] ">
                            ...
                            <div className="bg-[#EEFFFF]  p-4">{item.Joins.map((user) => (
                                ...
                                <div key={user.ID}><div className="flex  items-center justify-between" >
                                    ...
                                    {showInfo === user.Email && <div className="mt-2 font-normal text-[0.8em] text-black ">
                                        <p className=" text-gray-600">{user.Description}</p>
                                        <p className="">FaceBook: <span className=" text-gray-600 hover:underline"><a href={user.Link_1} target="blank">{user.Link_1 !== '' ? user.Link_1 : "-"}</a></span></p>
                                        <p className="">Instagram: <span className=" text-gray-600"><a href={user.Link_2} target="blank">{user.Link_2 !== '' ? user.Link_2 : "-"}</a></span></p>
                                        <p className="">other: <span className=" text-gray-600"><a href={user.Link_1} target="blank">{user.Link_3 !== '' ? user.Link_3 : "-"}</a></span></p>
